...
def monitor_chat():
    while True:
        if not chat_room.empty():
            msg_data = chat_room.get()
            encrypted_msg = msg_data['message']
            sender = msg_data['sender']
            decrypted_msg = decrypt_message(encrypted_msg)
            print(f"\n[CHAT] {sender}: {decrypted_msg}")
        time.sleep(0.5)
...
PORT = 65432
print(f"HTTP Chat Room Server berjalan di port {PORT}")
print("Siap menerima koneksi dari 2 client!")
print("Server berjalan sebagai relay (chat tidak ditampilkan di sini)")
print("-" * 50)
